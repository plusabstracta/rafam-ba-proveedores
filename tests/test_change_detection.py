...
from main import _sync_entity_changes
...
class TestSyncEntityChangesProveedores:
    ...
    def test_sync_changed_proveedores(self, setup_env):
        ...
        # Check that exporter temp hashes were populated
        assert hasattr(exporter, "_temp_proveedor_payload_hashes")
        assert "100" in exporter._temp_proveedor_payload_hashes
...
class TestSyncEntityChangesOcItems:
    ...
    def test_sync_oc_items_changed(self, setup_env):
        ...
        assert res is True
        source_repo.fetch_oc_items_by_keys.assert_called_once_with([{"ejercicio": 2026, "nro_oc": 100, "uni_compra": 1}])
    ...
    def test_sync_oc_items_unchanged(self, setup_env):
        ...
        row = self._oc_row()
        grouped, _, _, _ = exporter._group_oc_rows(self.OC_COLUMNS, [row])
