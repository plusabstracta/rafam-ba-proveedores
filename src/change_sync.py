...
from .config import _ejercicio_min_for
...
class ChangeSyncService:
    ...
    def _sync_oc_items(self, *, backfill_only: bool, dry_run: bool) -> bool:
        ...
        for i in range(0, len(oc_keys), self._OC_BATCH_SIZE):
            ...
            cols, rows = self._source_repo.fetch_oc_items_by_keys(chunk, ejercicio_min=ejercicio_min)
            if not columns and cols:
                columns = cols
            all_rows.extend(rows)

        # Agrupar OC filas en payloads (cabecera + items). Usa el mismo
        # group_rows del mapper que el path de envío → hashes idénticos.
        grouped, grouped_raw, grouped_gasto_refs, unresolved_items = (
            self._exporter._oc_mapper.group_rows(columns, all_rows)
        )

        # Índice source_key RAFAM → filas crudas (evita O(N²) al recolectar
        # las filas de cada OC modificada).
        rows_by_oc_key: dict[tuple[int | None, int | None, int | None], list[tuple]] = {}
        for row in all_rows:
            raw = dict(zip(columns, row))
            rk = (
            ...
            rows_by_oc_key.setdefault(rk, []).append(row)
        ...
        for key, oc_data in grouped.items():
            ...
            if backfill_only:
                ...
                to_send.extend(rows_by_oc_key.get(key, []))
