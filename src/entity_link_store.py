...
DEFAULT_LINK_SCHEMAS: dict[str, list[str]] = {
    "proveedores": ["cuit", "cod_estado", "payload_hash", "deleted_at"],
    ...
    # mercaderia: identidad RAFAM por descripcion normalizada (source_key
    # = "name:<normalized>"). Evita re-resolver contra el resolver remoto en
    # cada corrida y preserva el barcode/nombre_compra devuelto por Paxapos.
    "mercaderia": ["barcode", "nombre_compra", "deleted_at"],
...
class EntityLinkStore:
    ...
    def get_all_links(self, entity: str) -> list[dict]:
        """Devuelve todos los links (no anulados) de una entidad como dicts.

        Wrapper de ``iter_all_links`` usado por el flujo de detección de
        cambios (sync-changes). Excluye los registros con deleted_at.
        """
        if entity not in self._schemas:
            return []
        return self.iter_all_links(entity)
