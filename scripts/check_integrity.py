...
Detecta dos tipos de anomalías en los registros ya migrados:
  ...
  2. CAMBIOS DE CONTENIDO — solo proveedores
     ...
     en payload_hash del link store. Indica que algún campo cambió en RAFAM.
...
from src.change_detection import compute_payload_hash  # noqa: E402
...
from src.gateway_mapper import map_proveedor_migrator_row  # noqa: E402
...
def check_content_hash_proveedores(
    ...
    *,
    ...
    for link in links:
        source_key = link["source_key"]
        saved_hash = link.get("payload_hash")
        result.verificados += 1
        ...
        if not mapped:
            # Sin payload mapeable no hay hash comparable; lo trata anulaciones.
            result.sin_cambios += 1
            continue
...
def _diff_fields(link: dict, raw: dict) -> list[str]:
    """Detecta qué campos difieren (solo para logging descriptivo).

    Solo se almacena el hash previo (no los valores originales), por lo que no
    es posible identificar el campo exacto que cambió; se reporta la diferencia
    de hash de forma genérica.
    """
