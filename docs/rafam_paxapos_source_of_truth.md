...
| Migrator RAFAM | `PAXAPOS_RAFAM_IMPORT_PATH`, `PAXAPOS_RAFAM_SPEC_PATH`, `PAXAPOS_RAFAM_LOOKUPS_PATH`, `PAXAPOS_RAFAM_DEFAULT_UNIDAD_ID`, `PAXAPOS_RAFAM_DEFAULT_TIPO_FACTURA_ID`, `PAXAPOS_RAFAM_DEFAULT_TIPO_PAGO_ID`, `RAFAM_SYNC_BATCH_DELAY_SECONDS`, `RAFAM_EJERCICIO_MIN` |

`RAFAM_EJERCICIO_MIN` aplica solo a `pedidos`, `ped_items`, `orden_compra`, `oc_items`, `solic_gastos` y `orden_pago`. No filtra `proveedores`, `jurisdicciones` ni catalogos.
