...
| ORDEN_PAGO | `NRO_CANCE` | — | — | FK a RETENCIONES, NO nexo OP↔OC |
...
| ORDEN_PAGO | `RECO_DEU_COMPRA` | ordenes_pago | gasto_ids (indirecto) | Nexo OP↔OC: = NRO_OC de la OC pagada. Resuelve gasto_refs via entity_link_store. |
| ORDEN_PAGO | `RECO_DEU_COMPRA_EJER` | ordenes_pago | — | Ejercicio de la OC referenciada (parte de FK → ORDEN_COMPRA) |
