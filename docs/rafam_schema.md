...
> Generado automáticamente por `scripts/explore_schema.py` el 2026-03-17 19:43:44
...
**PK:** `EJERCICIO`, `NRO_OP`  
**FK:** `EJERCICIO` → `OWNER_RAFAM.ASIENTOS`, `EJERCICIO` → `OWNER_RAFAM.FUEN_FIN`, `COD_PROV` → `OWNER_RAFAM.PROVEEDORES`, `LUG_EMI` → `OWNER_RAFAM.LOCALIDADES`, `JURISDICCION` → `OWNER_RAFAM.JURISDICCIONES`, `CODIGO_UE` → `OWNER_RAFAM.UNI_EJEC`, `COD_EMP` → `OWNER_RAFAM.EMPRESTITOS`, `TIPO_DOC` → `OWNER_RAFAM.TIPO_DOC_RES`, `EJERCICIO` → `OWNER_RAFAM.ASIENTOS`, `ASIENTO_ANUL` → `OWNER_RAFAM.ASIENTOS`, `CODIGO_FF` → `OWNER_RAFAM.FUEN_FIN`, `ASIENTO` → `OWNER_RAFAM.ASIENTOS`  
...
**FK:** `LUG_EMI` → `OWNER_RAFAM.LOCALIDADES`, `EJERCICIO` → `OWNER_RAFAM.FUEN_FIN`, `JURISDICCION` → `OWNER_RAFAM.JURISDICCIONES`, `JURISDICCION` → `OWNER_RAFAM.DEPENDENCIAS`, `CODIGO_UE` → `OWNER_RAFAM.UNI_EJEC`, `CODIGO_DEP` → `OWNER_RAFAM.DEPENDENCIAS`, `CODIGO_FF` → `OWNER_RAFAM.FUEN_FIN`  
...
**FK:** `COD_LUG_ENT` → `OWNER_RAFAM.LUGARES_ENT`, `CODIGO_UE` → `OWNER_RAFAM.UNI_EJEC`, `TIPO_DOC` → `OWNER_RAFAM.TIPO_DOC_RES`, `JURISDICCION` → `OWNER_RAFAM.DEPENDENCIAS`, `EJERCICIO` → `OWNER_RAFAM.PEDIDOS`, `JURISDICCION` → `OWNER_RAFAM.JURISDICCIONES`, `DELEG_SOLIC` → `OWNER_RAFAM.DELEGACIONES`, `EJERCICIO` → `OWNER_RAFAM.FUEN_FIN`, `CODIGO_FF` → `OWNER_RAFAM.FUEN_FIN`, `NRO_PED` → `OWNER_RAFAM.PEDIDOS`, `CODIGO_DEP` → `OWNER_RAFAM.DEPENDENCIAS`  
